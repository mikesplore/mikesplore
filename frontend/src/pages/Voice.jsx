...
export default function Voice() {
  ...
  function displayItem(item) {
    setActions((current) => current.some((entry) => entry.url === item.url) ? current : [...current, item]);
  }
    ...
    {hasSheetContent && !sheetDismissed && <aside className={`fixed inset-x-0 bottom-0 z-20 overflow-y-auto rounded-t-3xl border-t border-divider bg-page/95 px-5 pb-8 pt-4 text-center shadow-2xl backdrop-blur transition-[max-height,transform] duration-300 sm:inset-x-auto sm:left-1/2 sm:w-full sm:max-w-2xl sm:-translate-x-1/2 lg:inset-y-0 lg:right-0 lg:bottom-auto lg:left-auto lg:h-screen lg:max-h-none lg:w-[clamp(22rem,34vw,42rem)] lg:max-w-none lg:translate-x-0 lg:rounded-none lg:rounded-l-3xl lg:border-l lg:border-t-0 lg:px-7 lg:pt-7 ${sheetExpanded ? 'max-h-[94vh]' : 'max-h-[72vh]'}`}>
      ...
      {actions.some((action) => action.type === 'display_item') && <div className="mx-auto w-full max-w-2xl space-y-6 text-left">
        {actionGroups().map(([group, items]) => <section key={group}>
          ...
          <div className={items.length === 1 ? 'flex justify-center' : 'grid grid-cols-2 gap-3 sm:grid-cols-3'}>{items.map((action) => <button type="button" key={`${action.label}-${action.url}`} onClick={() => pickMedia(action)} className={`${items.length === 1 ? 'w-full max-w-sm' : 'w-full'} overflow-hidden rounded-2xl border border-divider bg-card text-left shadow-sm transition hover:border-primary`}>{action.item_type === 'cv' ? <div className="flex h-40 items-center justify-center bg-black/5 text-5xl">📄</div> : <img src={action.url} alt={action.label} className={`${items.length === 1 ? 'h-56' : 'h-32'} w-full object-cover`} />}<span className="block truncate px-4 py-3 text-sm font-medium text-ink">{action.label}</span></button>)}</div>
        </section>)}
      </div>}

...
async def extract_admin_operation(instruction: str, candidates: list[dict] | None = None) -> dict:
    system = "Extract one admin portfolio CRUD operation. Return only JSON with resource (entries, certificates, assets, links, skills, education, bucket-list, settings, profile), action (create, update, delete), id (string or null), and payload (object). Never invent IDs or values. Use profile for profile field updates. If candidates contain multiple plausible records, return action null and include candidates in the response.\nCandidates:\n" + json.dumps(candidates or [])
    ...
    if result.get("action") is not None and (result.get("resource") not in {"entries", "certificates", "assets", "links", "skills", "education", "bucket-list", "settings", "profile"} or result.get("action") not in {"create", "update", "delete"}):

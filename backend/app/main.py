...
def search_portfolio(q: str = Query(min_length=1), page: int = Query(default=1, ge=1), page_size: int = Query(default=5, ge=1, le=50), db: Session = Depends(get_db)):
    term = f"%{q}%"
    terms = [word.lower() for word in re.findall(r"[a-z0-9]+", q.lower()) if len(word) > 2]
    query = select(Entry).where(Entry.is_visible.is_(True), or_(Entry.title.ilike(term), Entry.blurb.ilike(term), cast(Entry.tags, String).ilike(term), cast(Entry.source, String).ilike(term))).order_by(Entry.custom_order, Entry.date.desc().nullslast())
...
def list_projects(
    ...
    if category: query = query.where(Entry.category == category)
    if status: query = query.where(Entry.status == status)
    if featured is not None: query = query.where(Entry.is_featured.is_(featured))
    if technology:
...
@app.get("/api/v1/projects/{slug}/media")
...
@app.get("/api/v1/projects/{slug}/relationships")
def project_relationships(slug: str, db: Session = Depends(get_db)):
    ...
    for relation in db.scalars(select(Relationship).where(Relationship.subject_type == "entry", Relationship.subject_id == project.id)).all():
        ...
        if relation.object_type == "entry":
            ...
            display = None
        if display: result.append({"predicate": relation.predicate, "note": relation.note, "object_type": relation.object_type, "object": display})
    return result


@app.get("/entries/{entry_id}/content-blocks")
@app.get("/api/v1/entries/{entry_id}/content-blocks")

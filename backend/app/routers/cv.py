...
def get_cv_tailoring_context(db: Session = Depends(get_db)):
    ...
    data = cv_service.build_cv_data(db)
    projects = []
    for project in data.get("projects", [])[:15]:
        bullets = [str(bullet)[:300] for bullet in (project.get("bullets") or [])[:4]]
        projects.append({"id": cv_service.project_id(project), "name": project["name"], "date": project.get("date"), "stack": [item.strip() for item in (project.get("stack") or "").split(",") if item.strip()][:20], "bullets": bullets})
...
@router.post("/admin/cv/render-current", dependencies=[Depends(require_service_key)])
def render_current_cv(db: Session = Depends(get_db)):
    return cv_service.render_current_cv(db)

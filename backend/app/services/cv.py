...
def validate_cv_data(data: dict) -> dict:
    ...
    if not isinstance(data["certifications"], list):
        raise HTTPException(status_code=422, detail="CV certifications must be an array")
...
def save_cv_base(db: Session, data: dict, base_revision: str | None = None, portfolio_revision: str | None = None) -> dict:
    ...
    if portfolio_revision is not None:
        live_data = build_cv_data(db)
        if cv_base_hash(live_data) != portfolio_revision:
            raise HTTPException(status_code=409, detail="Portfolio data changed while this sync proposal was pending; run /synccv again")
    if setting:
        setting.value = data
    else:
        db.add(SiteSetting(key="cv_data", value=data))
    db.commit()
    return {"status": "saved", "revision": cv_base_hash(data)}
...
def build_cv_data(db: Session) -> dict:
    ...
    for project in project_rows:
        ...
        projects.append({
            ...
            "tags": [str(tag) for tag in (project.tags or [])],
        })
...
def render_cv(db: Session, payload: dict) -> dict:
    """Render a tailored CV from the reviewed CV JSON and store its snapshot."""
    base = get_cv_base(db)["data"]
    if payload.get("base_revision") != cv_base_hash(base):
        raise HTTPException(status_code=409, detail="Approved CV JSON changed while this proposal was pending; run /apply again")
    ...
    version_id = uuid4()
    return _render_and_store(db, data, label, job_description, patch, base, version_id)


def _render_and_store(db: Session, data: dict, label: str, job_description: str, patch: dict, base: dict, version_id=None) -> dict:
...
def render_base_cv(db: Session) -> dict:
    """Render the approved CV JSON as-is; this path does not call an LLM."""
    data = get_cv_base(db)["data"]
    return _render_and_store(db, data, "Current Portfolio CV", "Approved CV JSON", {}, data)
